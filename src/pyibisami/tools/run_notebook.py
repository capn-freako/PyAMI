...
def run_notebook(
    ...
    # Validate input.
    assert ibis_file.exists(), RuntimeError(f"Can't find IBIS-AMI model file, {ibis_file}!")
    assert notebook.exists(), RuntimeError(f"Can't find notebook file, {notebook}!")

    # Define temp. (i.e. - parameterized) notebook and output file locations.
    tmp_dir = (
        os.environ.get("TMP")
        or os.environ.get("TEMP")  # noqa: W504
        or (Path(os.environ.get("HOME")).joinpath("tmp") if os.environ.get("HOME") else "/tmp")  # type: ignore
    )
    ...
    try:
        ...
        if notebook_params:
            extra_args = [
                tok for item in notebook_params.items() for tok in ["-p", f"{item[0]}", f"{item[1]}"]
            ]  # noqa: E127
        ...
        raise
    subprocess.run(
        ["jupyter", "nbconvert", "--to", "html", "--no-input", "--output", html_file, tmp_notebook], check=True
    )
...
@click.command(context_settings={"ignore_unknown_options": False, "help_option_names": ["-h", "--help"]})
@click.option(
    "--notebook", "-n", default=NOTEBOOK, type=click.Path(exists=True), help="Override the default notebook file name."
)
@click.option(
    "--out-dir",
    "-o",
...
def main(
    notebook, out_dir, params, ibis_file, bit_rate, debug, is_tx, nspui, nbits, plot_t_max, f_max, f_step, fig_x, fig_y
):
    "Run a *Jupyter* notebook on an IBIS-AMI model file."
    run_notebook(
        Path(ibis_file).resolve(),
